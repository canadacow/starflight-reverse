...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float sphereRadius = 0.445; // Sphere (planet) radius
    ...
    if (discriminant < 0.0) {
        ...
        // Sample the texture
        vec3 albedo = textureBicubic(iChannel0, vec2(longitude,latitude)).rgb;
		...
		// Generate clouds
        float cloudIntensity = clamp(clouds(point, rayDir) - 0.8, 0.0, 1.0);
	...
	float atmosphereIntensity = 0.75; // Adjust this to change the intensity of the atmosphere
