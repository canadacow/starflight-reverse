...
DiligentShadowMapManager::DiligentShadowMapManager()
{
}

void DiligentShadowMapManager::Initialize(IRenderDevice* pDevice, IRenderStateCache* pStateCache, const InitInfo& initInfo)
{
...
void DiligentShadowMapManager::DistributeCascades(const DistributeCascadeInfo& Info,
...
void DiligentShadowMapManager::InitializeConversionTechniques(TEXTURE_FORMAT FilterableShadowMapFmt)
{
...
void DiligentShadowMapManager::InitializeResourceBindings()
{
    for (int mode = SHADOW_MODE_VSM; mode <= SHADOW_MODE_EVSM4; ++mode)
    {
        auto& Tech = m_ConversionTech[mode];
...
void DiligentShadowMapManager::ConvertToFilterable(IDeviceContext* pCtx, const HLSL::ShadowMapAttribs& ShadowAttribs)
{
    if (m_ShadowMode == SHADOW_MODE_VSM || m_ShadowMode == SHADOW_MODE_EVSM2 || m_ShadowMode == SHADOW_MODE_EVSM4)
    {
